 ...
 */
public class BubbleCover extends SurfaceView implements SurfaceHolder.Callback {

    private static final int EXPLOSION_SIZE = 50;
    private int mMaxDistance = 100;

    private ExplosionUpdateThread mThread;
    private BubbleExplosion mExplosion;
    ...
    public BubbleCover(Context context) {
        super(context);
    ...
    public void clearViews() {
        if (getParent() != null) {
            BubbleManager.getInstance().getWindowManager().removeView(this);
        }
    }
    ...
    public void initExplosion(float x, float y) {
        if (mExplosion == null || mExplosion.getState() == BubbleExplosion.STATE_DEAD) {
            mExplosion = new BubbleExplosion(EXPLOSION_SIZE, (int) x, (int) y);
        }
    }
    ...
    public class ExplosionUpdateThread extends Thread {
        ...
        private BubbleCover mDropCover;
        private boolean isRunning = false;

        public ExplosionUpdateThread(SurfaceHolder holder, BubbleCover dropCover) {
            mHolder = holder;
