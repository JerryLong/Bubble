...
public class Bubble extends RelativeLayout {
    ...
    private BubbleCover.OnDragCompeteListener mOnDragCompeteListener;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                mHolderEventFlag = !BubbleManager.getInstance().isRunning();
                if (mHolderEventFlag) {
                    if (parent != null)
                        parent.requestDisallowInterceptTouchEvent(true);
                    BubbleManager.getInstance().start(this, event.getRawX(),
                            event.getRawY(), mOnDragCompeteListener);
                }
                break;
            case MotionEvent.ACTION_MOVE:
                if (mHolderEventFlag) {
                    BubbleManager.getInstance().update(event.getRawX(),
                            event.getRawY());
                }
                break;
            ...
            case MotionEvent.ACTION_CANCEL:
                if (mHolderEventFlag) {
                    ...
                    BubbleManager.getInstance().finish(this, event.getRawX(),
                            event.getRawY());
                }
                break;
        }
    ...
    public void setOnDragCompeteListener(
            BubbleCover.OnDragCompeteListener onDragCompeteListener) {
